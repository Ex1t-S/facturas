...
export function EngineeringPage({ companyId }: { companyId: string }) {
  ...
  return <div className="engineeringPage">
    ...
    {tab === 'library' && <LibraryView companyId={companyId} />}
...
function EngineeringMessage({ item }: { item: AnyRecord }) {
  ...
  return <article className={`engineeringMessage ${isUser ? 'user' : 'assistant'}`}>{!isUser && <span className="assistantAvatar"><Sparkles size={14} /></span>}<div className="messageBody"><span className="messageRole">{isUser ? 'Vos' : 'FMH · Asistente'}</span><p>{item.content}</p>{!isUser && item.structuredResultJson && <TechnicalDetails result={item.structuredResultJson} />}</div></article>;
}
