...
export async function validateEngineeringBenchmark(benchmarkId: string, companyId = '') {
  ...
  const validation = await prisma.engineeringToolValidation.upsert({ where: { toolName_toolVersion_benchmarkId: { toolName: benchmark.implementedTool, toolVersion, benchmarkId } }, update: { companyId: benchmark.companyId, resultJson: JSON.stringify({ actual, expected, errors }), absoluteError: errors.length ? Math.max(...errors.map((item) => item.absoluteError)) : null, relativeError: errors.length ? Math.max(...errors.map((item) => item.relativeError)) : null, passed, scope: passed ? 'VALIDATED_FOR_SCOPE' : 'PARTIALLY_VALIDATED', validatedAt: new Date() }, create: { companyId: benchmark.companyId, toolName: benchmark.implementedTool, toolVersion, benchmarkId, resultJson: JSON.stringify({ actual, expected, errors }), absoluteError: errors.length ? Math.max(...errors.map((item) => item.absoluteError)) : null, relativeError: errors.length ? Math.max(...errors.map((item) => item.relativeError)) : null, passed, scope: passed ? 'VALIDATED_FOR_SCOPE' : 'PARTIALLY_VALIDATED' } });
